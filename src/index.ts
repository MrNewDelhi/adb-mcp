...
const serialSchema = z.string().min(1).optional().describe("ADB device serial. Omit to use adb's default target.");
const timeoutSchema = z.number().int().positive().max(3_600_000).optional().describe("Command timeout in milliseconds.");
const envSchema = z.record(z.string(), z.string()).optional().describe("Extra environment variables for the adb process.");
const optionalBool = z.boolean().optional();

function targetArgs(serial?: string): string[] {
  return serial ? ["-s", serial] : [];
}

async function runProcess(command: string, args: string[], timeoutMs = DEFAULT_TIMEOUT_MS, env?: Record<string, string>): Promise<RunResult> {
...
async function adb(args: string[], timeoutMs?: number, env?: Record<string, string>): Promise<RunResult> {
  return runProcess(adbBinary, args, timeoutMs, env);
}
...
function result(resultValue: RunResult, parse?: (stdout: string) => unknown) {
...
function parsePackageList(stdout: string) {
  return nonEmptyLines(stdout).map((line) => line.replace(/^package:/, ""));
}

function boolArg(enabled?: boolean): "enable" | "disable" | undefined {
...
function resolveOutputPath(path: string | undefined, extension: string): string {
  if (path) return isAbsolute(path) ? path : resolve(path);
  return join(tmpdir(), `adb-mcp-${randomUUID()}${extension}`);
}

function intentArgs(input: {
  action?: string;
  data?: string;
  mimeType?: string;
  component?: string;
...
server.tool("adb_version", "Show adb version and installation path.", {}, async () => result(await adb(["version"])));
...
server.tool("adb_intent", "Start an activity/service/foreground service or send a broadcast with intent fields and extras.", intentSchema, async (input) => {
...
server.resource("adb://cheatsheet", "adb://cheatsheet", async (uri) => ({
  contents: [
    {
      uri: uri.href,
...
server.prompt("adb_triage", "Collect an Android device/app triage plan.", { packageName: z.string().optional() }, ({ packageName }) => ({
  messages: [
    {
      role: "user",
      content: {
